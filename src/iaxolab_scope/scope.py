...
class Scope:
    def __init__(self, name: str = "SDS7AA1D7R0092"):
        ...
        if len(resources_filtered) > 1:
            raise ValueError(f"Multiple devices found with name {name}. Devices: {resources}")

        self._osc = self._rm.open_resource(resources_filtered[0])
        assert isinstance(self._osc, visa.resources.MessageBasedResource)
        self._osc: visa.resources.MessageBasedResource = self._osc
        ...
        print(f"Connected to '{identifier}' at '{str(self._osc)}'")
    ...
    def write(self, message: str, *args, **kwargs):
        self._osc.write(message, *args, **kwargs)
